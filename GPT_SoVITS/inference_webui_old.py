...
reference_wavs_pro = ["请选择参考音频或者自己上传"]
...
def change_choices_ref():
    ...
    return {"choices":reference_wavs_pro, "__type__": "update"}
...
def init_wav_list(sovits_path):
    ...
    match = re.search(r'([a-zA-Z0-9\-_\u4e00-\u9fff]+)_e\d+_s\d+\.pth',sovits_path)
    ...
    # 遍历目录
    for file_path in os.listdir(wav_path)[:150]:
        # 检查当前file_path是否为文件
...
def change_wav_model(audio_name):
...
def change_sovits_weights(sovits_path,prompt_language=None,text_language=None):
    ...
    if prompt_language is not None and text_language is not None:
        ...
        return  {'__type__':'update', 'choices':list(dict_language.keys())}, {'__type__':'update', 'choices':list(dict_language.keys())}, prompt_text_update, prompt_language_update, text_update, text_language_update,gr.update(choices=reference_wavs)
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False,speed=1,if_freeze=False,inp_refs=123):
    ...
    if not ref_free:
        with torch.no_grad():
            ...
            # if (wav16k.shape[0] > 160000 or wav16k.shape[0] < 28000):
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            wavs_dropdown_model = gr.Dropdown(label=i18n("模型参考音频列表"), choices=reference_wavs,value="请选择参考音频",interactive=True)
            refresh_button = gr.Button(i18n("刷新模型路径"), variant="primary", scale=14)
            refresh_button.click(fn=change_choices, inputs=[], outputs=[SoVITS_dropdown, GPT_dropdown])
            
        gr.Markdown(html_center(i18n("*请上传并填写参考信息"),'h3'))
        with gr.Row():
            wavs_dropdown = gr.Dropdown(label="参考音频列表",choices=reference_wavs_pro,value="请选择参考音频或者自己上传",interactive=True)
            refresh_button_ref = gr.Button("刷新参考音频")
            refresh_button_ref.click(fn=change_choices_ref, inputs=[], outputs=[wavs_dropdown])
        with gr.Row():
        ...
        SoVITS_dropdown.change(change_sovits_weights, [SoVITS_dropdown,prompt_language,text_language], [prompt_language,text_language,prompt_text,prompt_language,text,text_language,wavs_dropdown_model])
        GPT_dropdown.change(change_gpt_weights, [GPT_dropdown], [])

        wavs_dropdown_model.change(change_wav_model,[wavs_dropdown_model],[inp_ref,prompt_text])
